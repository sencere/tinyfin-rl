...
static tfrl_action a2c_act(void *ctx, tfrl_obs obs) {
    tfrl_a2c_algo *algo = (tfrl_a2c_algo *)ctx;
...
static void a2c_update(void *ctx, const tfrl_transition *transition) {
    tfrl_a2c_algo *algo = (tfrl_a2c_algo *)ctx;
...
static void a2c_destroy(void *ctx) {
    tfrl_a2c_algo *algo = (tfrl_a2c_algo *)ctx;
    if (!algo) return;
    adam_free(algo->opt);
...
static void a2c_save(void *ctx, const char *path) {
    tfrl_a2c_algo *algo = (tfrl_a2c_algo *)ctx;
    if (!algo) return;
...
static const tfrl_algo_vtable A2C_VTABLE = {
    .act = a2c_act,
    .act_batch = NULL,
    .update = a2c_update,
    .save = a2c_save,
...
tfrl_algo tfrl_algo_a2c_create(const tfrl_algo_config *cfg) {
    ...
    tfrl_a2c_algo *algo = (tfrl_a2c_algo *)calloc(1, sizeof(tfrl_a2c_algo));
    if (!algo) return out;
