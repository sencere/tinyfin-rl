...
void tfrl_cli_print_usage(const char *name) {
    ...
    fprintf(stderr, "  %s train --algo random|dqn|ppo|nca [--env NAME] [--envs N] [--threads N] [--steps N] [--seed N] [--gamma G] [--lr LR] [--epsilon E] [--deterministic]\n", name);
    ...
    fprintf(stderr, "  %s eval --algo random|dqn|ppo|nca [--env NAME] [--episodes N] [--seed N] [--gamma G] [--lr LR] [--epsilon E] [--deterministic]\n", name);
    ...
    fprintf(stderr, "  --entropy-coef N (ppo)\n");
    fprintf(stderr, "  --replay-size N --batch-size N --per-alpha N --per-beta N (dqn)\n");
    fprintf(stderr, "  --train-every N --learning-starts N --grad-steps N (dqn)\n");
    fprintf(stderr, "  --epsilon N (dqn/nca exploration)\n");
    fprintf(stderr, "  --action-bias-right N (discrete policies)\n");
    fprintf(stderr, "  removed v1 placeholder algos: rainbow, qrdqn, iqn, iql, a2c, a3c, trpo, sac, td3, impala, mcts\n");
...
int tfrl_cli_parse(int argc, char **argv, tfrl_runner_config *out_cfg) {
    ...
    out_cfg->algo = arg_str(argc, argv, "--algo", "ppo");
