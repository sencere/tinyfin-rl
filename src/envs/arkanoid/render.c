...
size_t tfrl_env_render_bytes_arkanoid(const tfrl_env *env) {
    if (!env) return 0;
    size_t bricks_bytes = (size_t)(ARKANOID_LINES * ARKANOID_BRICKS_PER_LINE);
...
size_t tfrl_env_render_write_arkanoid(tfrl_env *env, void *buffer, size_t buffer_len) {
    ...
    tfrl_arkanoid_snapshot payload = {
        ...
        .max_steps = (uint32_t)ARKANOID_MAX_STEPS,
        ...
        .brick_offset_y = ARKANOID_BRICK_OFFSET_Y,
        .lines = (uint32_t)ARKANOID_LINES,
        .bricks_per_line = (uint32_t)ARKANOID_BRICKS_PER_LINE,
    };
    ...
    for (int i = 0; i < ARKANOID_LINES; i++) {
        for (int j = 0; j < ARKANOID_BRICKS_PER_LINE; j++) {
            bricks_dst[i * ARKANOID_BRICKS_PER_LINE + j] = (unsigned char)(env->arkanoid.bricks[i][j] ? 1 : 0);
        }
    }
