...
size_t tfrl_env_render_bytes_floppy(const tfrl_env *env) {
    (void)env;
    return tfrl_render_grid_bytes(20, 15, 0);
}
...
size_t tfrl_env_render_write_floppy(tfrl_env *env, void *buffer, size_t buffer_len) {
    ...
    int ax = (int)((env->floppy.x / (float)FLOPPY_W) * (w - 1));
    int ay = (int)((env->floppy.y / (float)FLOPPY_H) * (h - 1));
    return tfrl_render_grid_write(env, buffer, buffer_len, w, h,
                                  (uint32_t)ax, (uint32_t)ay,
                                  (uint32_t)(w - 1), (uint32_t)(h - 1),
                                  (uint32_t)FLOPPY_MAX_STEPS, (uint32_t)env->kind,
                                  cells, 0, NULL);
}
