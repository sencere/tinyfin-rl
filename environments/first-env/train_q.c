...
static void env_reset(grid_env *env) {
    env->x = 0;
    env->y = 0;
    env->steps = 0;
...
static int env_state(const grid_env *env) {
    return env->y * GRID_SIZE + env->x;
}

static double env_step(grid_env *env, int action, int *done) {
    if (action == 0 && env->y > 0) {
        env->y -= 1;
    } else if (action == 1 && env->y < GRID_SIZE - 1) {
        env->y += 1;
    } else if (action == 2 && env->x > 0) {
        env->x -= 1;
    } else if (action == 3 && env->x < GRID_SIZE - 1) {
        env->x += 1;
    }
    env->steps += 1;
...
static int save_q_table(const char *path, double q[GRID_SIZE * GRID_SIZE][ACTIONS]) {
    FILE *f = fopen(path, "w");
    if (!f) {
        return 0;
    }
...
int main(int argc, char **argv) {
    ...
    int states = GRID_SIZE * GRID_SIZE;
    double q[GRID_SIZE * GRID_SIZE][ACTIONS];
