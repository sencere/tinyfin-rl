...
static void ppo_compute_probs(ppo_agent *agent) {
...
static int ppo_act(void *ctx, const tfrl_value *obs, tfrl_value *out_action) {
    ppo_agent *agent = (ppo_agent *)ctx;
    (void)obs;
    ppo_compute_probs(agent);
    ...
    agent->last_logp = log(agent->probs[action]);
    out_action->type = TFRL_VALUE_I64;
    out_action->as.i64 = action;
    return TFRL_OK;
}

static int ppo_observe(void *ctx, const tfrl_transition *transition) {
    ppo_agent *agent = (ppo_agent *)ctx;
    (void)transition->observation;
    double reward = transition->reward;
    double advantage = reward - agent->value;

    double old_logp = agent->last_logp;
    double new_logp = agent->last_logp;
    double ratio = exp(new_logp - old_logp);

    double clip_hi = 1.0 + agent->clip_eps;
    double clip_lo = 1.0 - agent->clip_eps;
    double clipped = ratio;
    if (advantage >= 0.0 && ratio > clip_hi) {
        clipped = clip_hi;
    } else if (advantage < 0.0 && ratio < clip_lo) {
        clipped = clip_lo;
    }

    double policy_scale = clipped * advantage;
    for (int i = 0; i < ACTIONS; i++) {
        double grad = (i == agent->last_action ? 1.0 : 0.0) - agent->probs[i];
        agent->logits[i] += agent->lr_policy * policy_scale * grad;
    }
