...
typedef struct {
    double weights[ACTIONS];
    double last_probs[ACTIONS];
    double lr;
    double baseline;
...
static int pg_act(void *ctx, const tfrl_value *obs, tfrl_value *out_action) {
    pg_agent *agent = (pg_agent *)ctx;
    (void)obs;
    double max_w = agent->weights[0];
    for (int i = 1; i < ACTIONS; i++) {
        if (agent->weights[i] > max_w) {
            max_w = agent->weights[i];
        }
    }
    double sum = 0.0;
    for (int i = 0; i < ACTIONS; i++) {
        double e = exp(agent->weights[i] - max_w);
        agent->last_probs[i] = e;
        sum += e;
    }
    for (int i = 0; i < ACTIONS; i++) {
        agent->last_probs[i] /= sum;
    }
    double r = rng_uniform(&agent->rng);
    double cdf = 0.0;
    int action = ACTIONS - 1;
    for (int i = 0; i < ACTIONS; i++) {
        cdf += agent->last_probs[i];
        if (r <= cdf) {
            action = i;
            break;
        }
...
static int pg_observe(void *ctx, const tfrl_transition *transition) {
    pg_agent *agent = (pg_agent *)ctx;
    int action = (int)transition->action.as.i64;
    double reward = transition->reward;
    double advantage = reward - agent->baseline;
    agent->baseline = (1.0 - agent->baseline_alpha) * agent->baseline + agent->baseline_alpha * reward;
    for (int i = 0; i < ACTIONS; i++) {
        double grad = (i == action ? 1.0 : 0.0) - agent->last_probs[i];
        agent->weights[i] += agent->lr * advantage * grad;
    }
    return TFRL_OK;
}
...
int main(void) {
    ...
    pg_agent agent_state = {.weights = {0.0, 0.0, 0.0}, .lr = 0.1, .baseline = 0.0, .baseline_alpha = 0.05, .rng = 1};
